class BoyLovePicaSource extends ComicSource {
    name = "香香腐宅";
    key = "boylove_cc";
    version = "1.0.0";
    minAppVersion = "4.0.0";
    ...
    baseUrls = ["https://boylove.cc", "https://www.boylove.cc"];
    activeBaseUrl = "https://boylove.cc";
    ...
    parseComicList(html) {
        ...
        for (let a of doc.querySelectorAll("a")) {
            ...
            if (!href || !/(comic|book|album|manga|detail|manhua|cartoon|read|boylove|bl)/i.test(href)) continue;
    ...
    explore = [{
        ...
        load: async (page) => {
            let paths = page <= 1
                ? ["/", "/comic", "/category", "/manhua", "/book"]
                : ["/page/" + page, "/comic/page/" + page, "/category/" + page, "/manhua/page/" + page, "/book/page/" + page];
    ...
    comic = {
        ...
        loadInfo: async (id) => {
            ...
            return { title: title || "香香腐宅", subTitle: "", cover: cover, description: this.text(descNode), tags: {}, chapters: chapters };
        },
